final class OscListener{
  ...
  float params[] = new float[ UPDATE_CNT*2];
  OscListener(){
    //コンストラクタ
    cnt = 0;
  ...
  void listen(OscMessage msg){
    ...
    if(msg.checkAddrPattern("/muse/elements/alpha_relative")){
      ...
      cnt++;
      if(cnt == UPDATE_CNT){
        for(int i=0;i<UPDATE_CNT;i++) System.out.print(params[i] + ",");
        System.out.println("");
        cnt = 0;
        int val_send;
        float data_ave = 0.0;
        ...
        data_ave /= ((float)UPDATE_CNT * (float)N_CHANNELS * (float)PARAM_MAX_VALUE ); //averageを[0,1)で正規化
        System.out.println("average : "+data_ave);
        val_send = (int)(data_ave * (float)PARAM_MAX_LEVEL) + 1;
        System.out.println("send  : "+val_send);
